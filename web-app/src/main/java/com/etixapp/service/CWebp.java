...
public class CWebp {
	 ...
	 */
	public CWebp(String webpConverterPath) {
		//
		//    	resourceLoader.getResource("classpath:" + "cwebp.exe").getURL().getPath();
		//    	ClassLoader classLoader = getClass().getClassLoader();
		//        File file = new File(classLoader.getResource("cwebp.exe").getFile());
		command = new StringBuffer();
		comm = new ArrayList<>();
		command.append(webpConverterPath + " ");
		comm.add(webpConverterPath);
	...
	public CWebp input(String inputPath) {
		command.append("\"").append(inputPath).append("\" ");
		comm.add(inputPath);
		return this;
	}
	...
	public void execute() {
		...
		try {
			process = Runtime.getRuntime().exec(comm.toArray(new String[0]));
			process.waitFor();
